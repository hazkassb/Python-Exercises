...
def great_():
    ...
    response = "That is a nice name" if name == "Hamza Kassomou" else "I like your name, let's be friends!" if name == "John Cleese" or name == "Michael Palin" else "You have a nice name"
    print(response)
...
def lotter_game():
    ...
    def is_match(num1, num2):
        '''checks whether num1 and num2 match'''
    ...
    user_digit1, user_digit2, user_digit3 = user_input[0], user_input[1], user_input[2]
    if user_input == str(lucky_number):
        print("Congrats! you won $10,000")
    elif is_match(lucky_number, user_input):
        print("Congrats! you won $3000")
    elif user_digit1 in str(lucky_number) or user_digit2 in str(lucky_number) or user_digit3 in str(lucky_number):
        print("Congrats! you won $1000")
    else:
        print("Sadly, you did not win anything. Try again!")
...
def find_max():
    sequence = input("Enter a sequence of space separated integers: ")

    arr = sequence.split()
    curr_max, count, i = float('-inf'), 0, 0
...
def toString():
    ...
    user_arr = list(usr_input.lower())
    ans = ""

    for i in range(len(user_arr)):
        if user_arr[i].isalpha():
            for key in dict.keys():
                if user_arr[i] in dict.get(key):
                    ans += str(key)
                    break
        else:
            ans += user_arr[i]
